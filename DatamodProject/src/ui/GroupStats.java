...
public class GroupStats extends JFrame {
	...
	DBController dbc;
	WorkoutList master;
	GroupStats(Timestamp start, Timestamp end, ExerciseGroup active, DBController dbc, WorkoutList master) {
		super("View Group statistics");
		this.start = start;
		this.end = end;
		this.dbc = dbc;
		this.master = master;
	...
	String generateStatString(ResultSet rs) {
		...
		return "<html><br><br>"
				...
				+ "Total intensity: " + sumint + "<br>"
				+ "Average performance: " + (Double.valueOf(sumperf)/dcount) + "<br>"
				+ "Average intensity: " + (Double.valueOf(sumint)/dcount) + "<br></html>";
	}
	void resetContent() {
		ResultSet rs = dbc.getGroupStats(active.id, start, end);
	...
	void setWList() {
		...
		for (WithGr wg : active.exercises) {
			for (WithEx we : wg.ex.workouts) {
